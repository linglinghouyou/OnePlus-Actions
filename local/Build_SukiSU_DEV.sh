...
clear
echo "==================================================="
echo "  SukiSU Ultra (DEV) OnePlus Kernel Build Configuration  "
echo "==================================================="
...
export CCACHE_DIR="$HOME/.ccache_${FEIL}_SukiSU_DEV"
...
for i in {1..3}; do
  KSU_API_VERSION=$(curl -fsSL "https://raw.githubusercontent.com/SukiSU-Ultra/SukiSU-Ultra/susfs-test/kernel/Makefile" | \
...
KSU_COMMIT_HASH=$(git ls-remote https://github.com/SukiSU-Ultra/SukiSU-Ultra.git refs/heads/susfs-test | cut -f1 | cut -c1-8)
KSU_VERSION_FULL="v${KSU_API_VERSION}-${KSU_COMMIT_HASH}-xiaoxiaow[DEV]"
...
v\\\$\$1-${KSU_COMMIT_HASH}-xiaoxiaow[DEV]
endef
...
git clone https://gitlab.com/simonpunk/susfs4ksu.git -b gki-${ANDROID_VERSION}-${KERNEL_VERSION}-dev
